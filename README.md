...
- **Pattern screens**: mono / CMYK / spot separation with each ink screened at its own angle, plus **palette mode** — a gradient map where the screen dithers the handoff between adjacent spot colours, so every band stays flat and prints in N screens. Per-ink solid plates for line art, adjustable black level. 30 threshold-field patterns (grid/dot/line lattices on square or hex packing, wave-warped variants, grain, petroglyph, Turing diffusion and dashes, carved marks, pebbles, ring pavers, plasma, three pointillisms, organic dots, blue-noise stipple). Rank-equalized fields keep tonal response linear and hold structure across the whole range — dots through the highlights, checkerboard at 50%, holes in the shadows. Output is binary, so the print PNG has **no semi-transparent pixels** (the knockout guarantee DTF/DTG RIPs need), and SVG export traces a 3× render for true vectors with knocked-out holes. Screens specify in **LPI** against the document's output size
