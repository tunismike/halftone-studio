...
- **Pattern screens**: 21 threshold-field patterns (grid/dot/line lattices, wave-warped variants, grain, petroglyph, pebbles, pavers, plasma, pointillism). Rank-equalized fields keep tonal response linear and hold structure across the whole range — dots through the highlights, checkerboard at 50%, holes in the shadows. Output is binary, so the print PNG has **no semi-transparent pixels** (the knockout guarantee DTF/DTG RIPs need), and SVG export traces a 3× render for true vectors with knocked-out holes. Screens specify in **LPI** against the document's output size
